...
struct TellUsAboutYouView: View {
    ...
    @State private var createdBusinessID: UUID? = nil
    ...
    // CHANGE: حاليًا التطبيق يشتغل بس على السوق المحلي (السعودية)، فحطينا
    // خيار "Market" بقيمة وحيدة "Local (Saudi Arabia)" مختارة افتراضيًا —
    // جاهز بالمستقبل نضيف له خيارات ثانية (Online / International) بدون
    // ما نغيّر التصميم أو نضيف حقل جديد من الصفر.
    @State private var selectedMarketScope = "Local (Saudi Arabia)"

    // Options Data
    let locationOptions = ["Riyadh", "Dammam", "Jeddah", "Not decided yet"]
    let budgetOptions = ["Under 50,000 SAR", "50,000 SAR - 100,000 SAR", "100,000 SAR - 300,000 SAR", "300,000 SAR - 500,000 SAR", "500,000 SAR+"]
    ...
    let marketScopeOptions = ["Local (Saudi Arabia)"]
    ...
    var body: some View {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    VStack(spacing: 14) {
                        ...
                        CustomPickerRow(iconType: .system("globe", Color("appGreen")), title: "Market", placeholder: "Select market", selection: $selectedMarketScope, options: marketScopeOptions)
                    ...
                    // الزر الأخضر ينشئ البزنس في الستور وينقل للشاشة التالية IdeaEvaluationView
                    Button(action: {
                        if isFormValid {
        ...
        .navigationDestination(isPresented: $navigateToEvaluation) {
            if let id = createdBusinessID {
                IdeaEvaluationView(businessID: id, onFinishCreation: onFinishCreation, onReject: onReject)
            }
        }
    }
}
